...
2. 对每种意图，生成 2 条面向监管条文检索的查询语句
3. 标注每条查询的意图类型，使用以下违规类型 ID：
   V01=承诺保本保息, V02=承诺确定收益, V03=绝对化用语, V04=无资质代言,
   "V05"=误导产品比较, "V06"=隐瞒淡化费用,
   "V07"=诱导退保转保, "V08"=伪造备案信息,
   "V09"=违规承诺增值服务
...
{{                                                                                                                                        
"queries": [                                                                                                                          
    {{"violation_type_id": "V01", "query": "违规意图的推理逻辑", "confidence": 0.9}}
]                                                                                                                                       
}}
...
class QueryRewriter:
            ...
            if matched:
                requests.append(
                    RewriteRequest(
                        ...
                        query_text=self._QUERY_TEMPLATES.get(
                            vid, vinfo["name"]),
                        keywords=matched,
                    )
                )
    ...
    def _call_llm(self, user_prompt: str) -> str:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                last_error = exc
                logger.warning(
                    "LLM rewrite attempt %d failed: %s", attempt, exc)
                if attempt < MAX_RETRIES:
                    time.sleep(2 ** (attempt - 1))

        logger.error("LLM rewrite failed after %d attempts: %s",
    ...
    def _parse_llm_response(self, raw: str) -> list[RewriteRequest]:
        try:
            ...
            logger.warning(
                "Failed to parse LLM rewrite response: %s | raw=%r", exc, raw[:200])
            return []
